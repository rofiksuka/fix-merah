...
import { getPendingFixes, updatePendingFix, getPendingFixByNumber } from './_lib/pending-fixes.js';
  ...
  return new Promise((resolve) => {
    ...
    imap.once('ready', () => {
      imap.openBox('INBOX', false, (err) => {
        if (err) {
          console.error(`OpenBox error for ${email}:`, err);
          imap.end();
          return resolve([]);
        }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`Checking ${senders.length} senders...`);
    
    for (const sender of senders) {
      try {
        console.log(`Checking inbox: ${sender.email}`);
        const replies = await checkSenderInbox(sender);
        
        if (replies.length > 0) {
          console.log(`Found ${replies.length} replies for ${sender.email}`);
        }
        ...
        for (const reply of replies) {
          ...
          if (pending) {
            ...
            const notified = await sendTelegramNotif(pending.user_id, reply.number, pending.fix_id);
            ...
            console.log(`â Notified user ${pending.user_id} for number ${reply.number}`);
          }
        }
      } catch (error) {
